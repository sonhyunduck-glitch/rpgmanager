...
// ── shopItemData.ts에서 food/gem 카테고리 전체 삭제 ──
let shop = fs.readFileSync('src/data/shopItemData.ts', 'utf8');

const delIds = [];
const foodRe = /\{ id: '(e_\d+)',.*?category: 'food' \}/g;
const gemRe = /\{ id: '(e_\d+)',.*?category: 'gem' \}/g;
let m;
while ((m = foodRe.exec(shop)) !== null) delIds.push(m[1]);
while ((m = gemRe.exec(shop)) !== null) delIds.push(m[1]);

console.log(`삭제 대상: food+gem ${delIds.length}개`);
